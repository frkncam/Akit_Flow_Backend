...
/**
 * Generates a PDF for a contract using a stored template:
 *   1. Loads contract + template (both org-scoped).
 *   2. Resolves {{key}} placeholders from the template's variable metadata
 *      (system bindings against the contract, defaultValue fallback) plus any
 *      caller-supplied customValues.
 *   3. Runs the substituted HTML through Thymeleaf so legacy ${contract.*}
 *      templates keep working alongside the new placeholder syntax.
 *   4. Converts HTML → PDF bytes via openhtmltopdf.
 *   5. Uploads PDF to MinIO; registers a new ContractFile version.
 */
@Service
...
public class ContractPdfService {
    ...
    /**
     * Flatten the contract view into the dotted system-binding keys exposed by
     * {@link TemplateSystemVariables} (e.g. "contract.title", "generatedAt").
     * Null fields become empty strings so a missing optional renders as "" not "null".
     */
